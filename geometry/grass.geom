...
void drawPrism(vec3 V1, vec3 V2, vec3 V3, vec3 N0) {
  ...
  EndPrimitive();
	...
	/*//CUADRADO ???? SI FUNCIONA GUAAAAAAU!!!!
	gl_Position = modelViewProjectionMatrix*vec4(A1, 1);
		EmitVertex();
	gl_Position = modelViewProjectionMatrix*vec4(A2, 1);
		EmitVertex();
...
void main(void) {
  ...
  drawPrism(gl_in[0].gl_Position.xyz,
	...
	/*//CUADRADO PARED --- el orden depende mucho!!!!
	gl_Position = modelViewProjectionMatrix*vec4(gl_in[1].gl_Position.xyz+d*N, 1);
		EmitVertex();
	gl_Position = modelViewProjectionMatrix*gl_in[1].gl_Position;
		EmitVertex();
	gl_Position = modelViewProjectionMatrix*vec4(gl_in[0].gl_Position.xyz+d*N, 1);
		EmitVertex();
		gl_Position = modelViewProjectionMatrix*gl_in[0].gl_Position;
		EmitVertex(); 
