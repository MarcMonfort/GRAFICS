...
void SeleccioSimple::postFrame() 
{
	...
	int aux = scene()->selectedObject();	//otra forma

    if (0 <= objSeleccionat & objSeleccionat < nObjectes)
    {
      Object& obj  = scn->objects()[aux];	//o objSeleccionat
...
void SeleccioSimple::keyPressEvent (QKeyEvent *event)
{
  ...
  // Assigna valor a la variable objSeleccionat
	...
	if (event->key() == Qt::Key_0) objSeleccionat = 0;
	if (event->key() == Qt::Key_1) objSeleccionat = 1;
	if (event->key() == Qt::Key_2) objSeleccionat = 2;
	if (event->key() == Qt::Key_3) objSeleccionat = 3;
	if (event->key() == Qt::Key_4) objSeleccionat = 4;
	if (event->key() == Qt::Key_5) objSeleccionat = 5;
	if (event->key() == Qt::Key_6) objSeleccionat = 6;
	if (event->key() == Qt::Key_7) objSeleccionat = 7;
	if (event->key() == Qt::Key_8) objSeleccionat = 8;
	if (event->key() == Qt::Key_9) objSeleccionat = 9;


	Scene* scn    = scene();
    int nObjectes = scn->objects().size();

	//if (objSeleccionat > nObjectes) objSeleccionat = -1;

	//tutoria
	//int seleccionat = scene()‐>selectedObject();
	scene()->setSelectedObject(objSeleccionat);
